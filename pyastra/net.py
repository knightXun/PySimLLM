...
def p2pSend(src, dst, packetSize, send_lat=3000):
    global n
    nvls_on = True
    import pdb; pdb.set_trace()
    port = portNumber[src][dst] 
    portNumber[src][dst]  = portNumber[src][dst]  + 1
    pg = 3
    dport = 100
    # send_lat = 6000
    # send_lat = 3 * 1000 
    ...
    win = 0
    if n.Get(src) not in pairBdp or n.Get(dst) not in pairBdp[n.Get(src)]:
        if not has_win:
            win = 0
        elif global_t == 1:
            win = maxBdp
        ...
        if not has_win:
            ...
            win = pairBdp[ n.Get(src) ][ n.Get(dst) ] 

    rtt = 0
    if src in pairRtt and dst in pairRtt[src]:
        rtt = maxRtt if global_t == 1 else pairRtt[src][dst]
    else:
        rtt = maxRtt if global_t == 1 else 0
